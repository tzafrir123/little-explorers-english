    ...
    <div className="min-h-screen flex flex-col items-center pt-8 p-4" dir="rtl">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          {/* Placed letters (wrong = red, correct = green, playing = blue) */}
          <div className="flex gap-2 min-h-[60px] justify-center flex-wrap" dir="ltr">
            {targetWord.split("").map((_, i) => {
              const letter = status === "wrong" ? wrongPlaced[i] : placed[i];
              return (
                <motion.div
                  key={i}
          ...
          {status === "wrong" && (
            ...
            >
              <p className="text-sm font-bold text-grass-foreground">✅ התשובה הנכונה:</p>
              <div className="flex gap-2 justify-center flex-wrap" dir="ltr">
                {targetWord.split("").map((letter, i) => (
                  <div
                    key={i}
                    className="w-12 h-12 rounded-xl border-2 bg-grass text-grass-foreground border-grass flex items-center justify-center text-2xl font-extrabold"
                  >
                    {letter}
                  </div>
                ))}
